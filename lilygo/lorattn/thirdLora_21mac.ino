...
void setup() {
  ...
  //=====
  Serial2.begin(4800, SERIAL_8N1, 12, 13); 
  ...
  // instead of reading any real sensor, we just generate some random numbers as example
   ...
   uint8_t value2 = radio.random(100);
   uint8_t value3 = radio.random(100);
   //uint16_t value2 = radio.random(2000);
//=============================================

  //====
   Serial.println("Requesting data... ");
   Serial2.println("r"); // Request data from Nano
   //
     unsigned long startTime = millis(); // Record the start time
  bool serTimeout=0;
  while (!Serial2.available() && (serTimeout==0)) { 
    // Wait for data on Serial2
    if (millis() - startTime >= 5000) { // Check if timeout has occurred
      Serial.println("Timeout occurred!"); 
      serTimeout=1;
      //goto timeout;//return; // Exit the loop if timeout
    }
    delay(1);
  }
  //
  ...
  //timeout:
   ...
   if (Serial2.available() > 0) {
    ...
    var1 = data.substring(0, comma1).toInt();
    var2 = data.substring(comma1 + 1, comma2).toInt();
    var3 = data.substring(comma2 + 1).toInt();
   ...
   testdata:
    ...
    //int moisturePercentage = map(moistureValue, moisCal, 0, 0, 100); 

      //uint8_t value1 = moisturePercentage; //radio.random(100);
      //uint16_t value2 = moistureValue;  //radio.random(2000);
      Serial.print("value1: ");
  ...
  uplinkPayload[1] = value2;   // See notes for high/lowByte functions
  uplinkPayload[2] = value3;
  // perform an uplink
